...
function scrambleTag(element) {
	var elements = document.getElementsByTagName(element);
	for (var elementCount = 0; elementCount < elements.length; elementCount++) {
		elements[elementCount].classList.remove("text-shadow-effect");
		var scrambledText = '';
		var words = elements[elementCount].textContent.split(' ');
		for (var wordCount = 0; wordCount < words.length; wordCount++) {
			randomNumber = Math.random();
			var word = words[wordCount];
			if (word.charCodeAt(0) > 255) {
				word = luni.tools.bent.decode(word);
			}
			if (randomNumber > 0.98) {
				word = luni.tools.mirror.encode(word)
			} else if (randomNumber > 0.75) {
				word = luni.tools.bent.encode(word)
			}
			scrambledText += word + ' '
		}
		if (randomNumber > 0.7) {
			elements[elementCount].classList.add("text-shadow-effect")
		}
		elements[elementCount].textContent = scrambledText.trim()
	}
}
...
function randomBGImage() {
	document.body.classList.remove('bg-image-1', 'bg-image-2', 'bg-image-3', 'bg-image-4', 'bg-image-5', 'bg-image-6', 'bg-image-7', 'bg-image-8');
...
randomBGImage();
setInterval(randomBGImage, 9000);